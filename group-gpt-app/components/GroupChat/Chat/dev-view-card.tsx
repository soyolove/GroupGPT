...
export default function DevViewChart({agentMember,initialMessages,ongoingMessages,speakingOrder}:AgentChatProps){
    return (
        <div>
            <Card className="fixed right-0 top-1/2 transform -translate-y-1/2">
                <CardContent className="gap-2">
                    ...
                    {ongoingMessages.map((message) => (
                        message.agent ? <div>{message.agent.name}</div> : <div>User</div>
                    ))}
